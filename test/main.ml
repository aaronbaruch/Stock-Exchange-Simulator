(* Test plan for Stocks: The test plan for Stocks is a system of combination of
   ...
   planned and works out as intended. TALK ABOUT HOW WE CANT TEST EVERYTHING
   BECuase SOME HAVE WIERD OUTPUTS LIKE NEWS, as we try to get as CLOSE TO MAX
   Coverage *)
...
let user_cli_suite =
  [
    ...
    >:: int_test 1 (User.UserImpl.get_day (User.UserImpl.next_day empty_user));
  ]
