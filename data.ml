...
(* Updated Data module type using the record *)
module type Data = sig
  val get_ticker_price : string * int -> string Lwt.t
  val calculate_stock_correlation : stock_query -> stock_query -> float Lwt.t
...
let get_ticker_price stock_query =
  let symbol, days_back = stock_query in
  if days_back = -1 then
    Lwt.fail_with "Fetching ticker price for -1 days back is not supported"
  else
    ...
    | `OK -> (
        try
          ...
          let get_most_recent_close json =
            (* existing code to parse JSON and get the most recent close price *)
          in
  ...
  let get_historical_prices stock_query days =
    let symbol, days_back = stock_query in
    let uri = Uri.of_string base_url in
    let params =
      [
  ...
  let calculate_stock_correlation query_1 query_2 days =
    let symbol1, days_back1 = query_1 in
    let symbol2, days_back2 = query_2 in
